...
function me.IsTitleKnown ( ID )
	return ID == CLEAR_TITLE or IsTitleKnown( ID ) -- Note: IsTitleKnown(-1) crashes the client
end
--[[****************************************************************************
...
function Options.OnLoad ()
	...
	Dropdown:SetPoint( "TOP", IconSelect, "BOTTOM", 0, 6 )
	...
	Button:SetScript( "OnEnter", function (self) ItemRack.OnTooltip(self) end )
	Button:SetScript( "OnLeave", function (self) GameTooltip:Hide(self) end )
...
do
	...
	function Options:DropdownInitialize ()
		...
		Info.text = _G.NONE -- Add no title option
